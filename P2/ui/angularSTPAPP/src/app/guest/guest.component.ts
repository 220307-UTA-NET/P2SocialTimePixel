...
async createGuest(colorinput : string)
{
 ...
 // this.httpClient.post<any>(`https://localhost:7161/api/Guest/ipaddress/{ip}`, (this.currentGuest.ipAddress)).subscribe();
...
async getGuest(ip:any) : Promise<void>
{
 ...
 // this.httpClient.get<Guest>(`https://localhost:7161/api/Guest`).pipe(map(response => response)).subscribe(response => {  
